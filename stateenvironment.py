...
class StateEnvironment(Environment):
    ...
    def is_frozen(self, state, location, previous=None):
        ...
        for i in range(len(neighbors)):
            ...
            if state_map[neighbor] == State.WALL and state_map[next_neighbor] == State.WALL:
                ...
                frozen_neighbor = True if (neighbor, location_tuple) in previous else self.is_frozen(state, np.array(neighbor), previous)
                frozen_next_neighbor = True if (next_neighbor, location_tuple) in previous else self.is_frozen(state, np.array(next_neighbor), previous)
    ...
    def is_dead_diagonal(self, state, location):
        ...
        for i in range(len(diagonals)):
            ...
            #print(f"diagonal neighbor:{state_map[diagonal_neighbor] > State.PLAYER}, diagonal_next_neighbor:{}, across:{}")

            # rect = patches.Rectangle((diagonal_neighbor[0]+0.5, diagonal_neighbor[1]+0.5), -1, -1, linewidth=0.5, edgecolor='deepskyblue', facecolor='deepskyblue')
            # ax.add_patch(rect)
            # rect = patches.Rectangle((diagonal_next_neighbor[0]+0.5, diagonal_next_neighbor[1]+0.5), -1, -1, linewidth=0.5, edgecolor='deepskyblue', facecolor='deepskyblue')
            # ax.add_patch(rect)
            # if self.is_valid(across):
            #     rect = patches.Rectangle((across[0]+0.5, across[1]+0.5), -1, -1, linewidth=0.5, edgecolor='deepskyblue', facecolor='deepskyblue')
            #     ax.add_patch(rect)
            ...
            if self.is_valid(across) and state_map[tuple(center)] == State.EMPTY:
                if state_map[diagonal_neighbor] > State.PLAYER and state_map[diagonal_next_neighbor] > State.PLAYER and state_map[across] > State.PLAYER: #hack for checking if its box or wall
                    ...
                    if ((state_map[tuple(possible_corner1)] > State.PLAYER and state_map[tuple(opposite_corner1)] > State.PLAYER) or (state_map[tuple(possible_corner2)] > State.PLAYER and state_map[tuple(opposite_corner2)] > State.PLAYER)):
